...
Prints `✓ Ready` in a second or two. Leave it running.

**After changing app code**, build the agent bundle first (this is safe to run while the old server
is still serving — it writes to a different directory):
...
> **Tell Hermes to re-query.** It will happily answer from its own earlier context and report the
> old numbers. Ask it explicitly to call `GET /api/v1/stats` again and quote `totalPosts` and
> `lastScrapedAt` back to you, so you can see it really re-fetched.

> **Filter by author, not by text.** `?authors=basiakubicka` returns exactly the posts that account
> wrote. A text search for a name also matches posts that merely mention it.
